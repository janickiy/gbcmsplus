...
class BannerValuesDynamicFormWidget extends \yii\base\Widget
{
  const WIDGET_NAME = 'bannerdynamicform';

  public $categoryPropId;
  public $categoryPropField;
  ...
  public function init()
  {
    ...
    if (empty($this->categoryPropId)) {
      throw new InvalidConfigException("The 'categoryPropId' property must be set.");
    }

    if (empty($this->categoryPropField)) {
      throw new InvalidConfigException("The 'categoryPropField' property must be set.");
    }
  ...
  {
    BannerDynamicFormAsset::register($view);
    ...
    // when added - assign page_prop_id value
    $js = 'jQuery(".' . $this->widgetContainer . '").on("afterInsert", function(e, newClone) {' . "\n";
    $js .= "    e.preventDefault();\n";
    $js .= "    var cloneBody = $(e.currentTarget).find('" . $this->widgetBody . "'); \n";
    $js .= "    cloneBody.find('" . $this->categoryPropField . "').val(" . $this->categoryPropId . "); \n";
    $js .= "});\n";
    $view->registerJs($js, $view::POS_READY);
  }
