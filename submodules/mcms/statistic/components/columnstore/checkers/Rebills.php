 ...
 * Проверяем ребиллы
 */
class Rebills extends BaseChecker
{
  ...
  public function getInnoDbCount()
  {
    return (int) Yii::$app->db
      ->createCommand('select count(1) from subscription_rebills where date between :dateFrom AND :dateTo')
      ->bindValues([
  ...
  public function getColumnStoreCount()
  {
    return (int) Yii::$app->dbCs
      ->createCommand('select count(*) from subscription_rebills where date between :dateFrom AND :dateTo')
      ->bindValues([
        ':dateFrom' => $this->dateFrom,
        ':dateTo' => $this->dateTo,
      ])
