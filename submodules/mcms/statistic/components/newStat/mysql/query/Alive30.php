...
/**
 * Находим какие пдп проявили активность за последние 30дней
 * (активность = факт ребилла, у и отсутствие отписки)
 */
class Alive30 extends BaseQuery
{
  ...
  public function init()
  {
    ...
    $this->from(['st' => 'alive30_ons_day_group']);
  ...
  {
    return [
      'revshare_alive30_ons' => 'SUM(revshare_alive30_ons)',
      'to_buyout_alive30_ons' => 'SUM(to_buyout_alive30_ons)',
    ];
  }
  /**
   * @inheritdoc
  ...
  public function handleFilterByDates($dateFrom, $dateTo, $ltvDateTo)
  {
    ...
    if (Yii::$app->formatter->asDate('today', 'php:Y-m-d') === $dateTo) {
  ...
  public function handleInitial(FormModel $formModel)
  {
    ...
    if (in_array(Group::BY_WEEK_NUMBERS, $formModel->groups, true)) {
      $this->andWhere([
        'or',
        ['st.date' => $formModel->getDateTo()],
        new Expression('DAYOFWEEK(st.date) = 1')
      ]);
    }
    ...
    if (in_array(Group::BY_MONTH_NUMBERS, $formModel->groups, true)) {
      $this->andWhere([
        'or',
        ['st.date' => $formModel->getDateTo()],
        ['st.date' => new Expression('LAST_DAY(st.date)')],
      ]);
    }
  }
