...
class Hits extends BaseQuery
{
  ...
  protected function getFieldList()
  {
    return [
      ...
      'tb' => 'SUM(st.count_tb)',
      'unique' => 'SUM(st.count_uniques)',
      'cpa_hits' => 'SUM(IF(st.is_cpa = 1, st.count_hits, 0))',
      'cpa_tb' => 'SUM(IF(st.is_cpa = 1, st.count_tb, 0))',
      'cpa_unique' => 'SUM(IF(st.is_cpa = 1, st.count_uniques, 0))',
      'revshare_hits' => 'SUM(IF(st.is_cpa = 0, st.count_hits, 0))',
      'revshare_tb' => 'SUM(IF(st.is_cpa = 0, st.count_tb, 0))',
      'revshare_unique' => 'SUM(IF(st.is_cpa = 0, st.count_uniques, 0))',
      'otp_hits' => 'SUM(IF(lst.code="onetime", st.count_hits, 0))',
      'otp_tb' => 'SUM(IF(lst.code="onetime", st.count_tb, 0))',
      'otp_unique' => 'SUM(IF(lst.code="onetime", st.count_uniques, 0))',
    ];
  }
  /**
   * @inheritdoc
