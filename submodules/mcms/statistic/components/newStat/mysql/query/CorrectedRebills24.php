...
/**
 * Достаем инфу по скорректированным ребиллам за сутки из таблицы subscription_rebills_corrected
 */
class CorrectedRebills24 extends BaseQuery
{
  ...
  public function init()
  {
    ...
    $this->from(['st' => 'subscription_rebills_corrected'])
      ->leftJoin(['s' => 'subscriptions'], "s.hit_id = st.hit_id AND
           (({$this->getTrialOperatorsInCondition('s.operator_id', true)} AND s.date = st.date)
            OR 
            ({$this->getTrialOperatorsInCondition('s.operator_id')} AND s.date = date_add(st.date, INTERVAL -1 DAY)))")
      ->andWhere(['is_fake' => 0]);
  }
  ...
  {
    return [
      'revshare_rebills24_corrected' => 'COUNT(s.id)',
    ];
  }
