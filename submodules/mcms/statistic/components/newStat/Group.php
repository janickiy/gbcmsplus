...
class Group
{
  ...
  const BY_LANDING_CATEGORIES = 'landingCategories';
  const BY_OFFER_CATEGORIES = 'offerCategories';
  const BY_WEBMASTER_SOURCES = 'webmasterSources';
  const BY_LINKS = 'arbitraryLinks';
  const BY_ALL_SOURCES = 'sources';
  ...
  const BY_SUBID_1 = 'subid1';
  const BY_SUBID_2 = 'subid2';

  const TRANSLATE_GROUP_BY_PREFIX = 'statistic.new_statistic_refactored.group-by-';
  const TRANSLATE_GROUP_PREFIX = 'statistic.new_statistic_refactored.group-';
  ...
  public function __construct($type, $value, FormModel $formModel)
  {
    ...
    $groupFormatter = Yii::createObject(
      'mcms\statistic\components\newStat\mysql\groupFormats\\' . Inflector::camelize($type),
      [$value, $formModel]
    );
  ...
  public static function getGroupsConfig()
  {
    return [
      ...
      self::BY_LANDING_CATEGORIES => ['permissionCheckMethod' => 'canGroupByLandings'],
      self::BY_OFFER_CATEGORIES => ['permissionCheckMethod' => 'canGroupByLandings'],
      self::BY_WEBMASTER_SOURCES => ['permissionCheckMethod' => 'canGroupBySources'],
      self::BY_LINKS => ['permissionCheckMethod' => 'canGroupBySources'],
      self::BY_ALL_SOURCES => ['permissionCheckMethod' => 'canGroupBySources'],
  ...
  /**
   * Название колонки для группировки. Например "Даты"
   * @param $key
   * @return string
   */
  ...
  /**
   * Название группировки . Например "По датам"
   * @param $key
   * @return string
   */
  public static function getGroupByLabel($key)
  {
    return Yii::_t(self::TRANSLATE_GROUP_BY_PREFIX . $key);
  }
  ...
  /**
   * Строка с данными для поиска по текущей группировке.
   * Для потоков, партнеров и т.д. вернется id, для дат, месяцев, недель - диапазон
   * @return string
   */
  public function getFilterBy()
  {
    return $this->groupFormatter->getValue();
  }
  ...
  {
    return $this->groupFormatter->getFormattedPlainValue();
  }
}
