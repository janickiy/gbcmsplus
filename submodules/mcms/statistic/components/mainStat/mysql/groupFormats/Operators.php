...
class Operators extends BaseGroupValuesFormatter
{
  ...
  public function getFormattedValue()
  {
    if (array_key_exists($this->value, self::$cached)) {
      return Yii::$app->formatter->asStringOrNull($this->replaceTemplate(self::$cached[$this->value]));
    }

    self::cacheBufferedValues();

    return Yii::$app->formatter->asStringOrNull($this->replaceTemplate(ArrayHelper::getValue(self::$cached, $this->value)));
  }
