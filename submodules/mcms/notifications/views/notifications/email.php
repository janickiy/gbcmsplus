...
<?php Pjax::begin(['id' => 'emailNotificationsPjaxGrid']); ?>
  ...
  'columns' => [
    [
      ...
      'filter' => UserSelect2::widget([
          'model' => $searchModel,
          'attribute' => 'to_user_id',
          'initValueUserId' => $searchModel->to_user_id,
          'options' => [
            'placeholder' => '',
          ],
        ]
    ...
    [
      ...
      'visible' => \Yii::$app->user->can('NotificationsNotificationsEmailNotOwn'),
    ],
    'header',
    [
      'attribute' => 'is_send',
      ...
      'value' => function ($model) {
        return ($model->notifications_delivery_id ?
          ...
          Modal::widget([
            ...
            'url' => Url::to(['/notifications/notifications/email-view-modal/', 'id' => $model->id]),
          ]);
      },
