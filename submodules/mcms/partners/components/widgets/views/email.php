...
<div style="background-color: #f4f4f3; width: 100%;">
  <div style="width: 100%; max-width: 700px; margin: 0 auto; background-color: #f4f4f3;">
    ...
    <div style="background-color: #373737; padding: 15px 20px;">
      <div style="overflow: hidden; margin-bottom: 10px;">
        <?php if ($logoUrl = $partnersModule->api('getLogoEmailImage')->getResult()): ?>
          <div style="float: left; display: inline-block; vertical-align: middle; width: auto; height: auto; max-width: 45%; max-height: 40px;">
        ...
        <div style="float: right; display: inline-block; vertical-align: middle; width: auto; height: auto;">
          <a target="_blank" href="<?= $partnersModule->getServerName(); ?>" style="color: #9d9d9d; font-family: Arial, Helvetica, sans-serif; font-size: 14px; line-height: 18px; text-decoration: none;">
            <?= str_replace(['http://', 'https://'], '', $partnersModule->getServerName()); ?>
          </a>
        </div>
      </div>
      <div style="margin-bottom:10px;">
        <strong style="font-family: Arial, Helvetica, sans-serif; font-size: 22px; line-height: 30px; color: #fff;">
          <?= $title; ?>
        </strong>
      </div>
      <div>
        <p style="margin: 0; font-family: Arial, Helvetica, sans-serif; font-size: 14px; line-height: 15px; color: #9d9d9d; background-image: url(http://resourcesfiles.com/panel_delivery/date.png); background-repeat: no-repeat; background-position: 0 50%; padding-left: 20px;">
          <?= Yii::$app->getFormatter()->asPartnerDate(time()); ?>
        </p>
      </div>
    </div>
    <div style="background-color: #3598db; height: 7px;"></div>
    <div style="font-size: 14px; font-family: Arial, Helvetica, sans-serif; padding: 30px 25px; border-left: 1px solid #d9d9d9; border-right: 1px solid #d9d9d9; background-color: #fff;">
      <?= $text; ?>
    </div>

    <?php if (ArrayHelper::keyExists('mainQuestions', $footerContactValues) || ArrayHelper::keyExists('techSupport', $footerContactValues)): ?>
      <div style="background-color: #3598db; height: 7px;"></div>
            ...
            <?php if ($footerMainQuestionSkype || $footerMainQuestionIcq): ?>
              <div>
                <?php if ($footerMainQuestionSkype): ?>
                  <a target="_blank" href="skype:<?= $footerMainQuestionSkype; ?>" style="display: inline-block; vertical-align: middle; text-decoration: none; font-family: Arial, Helvetica, sans-serif; font-size: 12px; line-height: 20px; color: #8b8888; background-image: url(http://resourcesfiles.com/panel_delivery/skype.png); background-repeat: no-repeat; background-position: 0 50%; padding-left: 25px; margin: 0 20px 20px 0;">
    ...
    <div style="text-align: center; padding: 20px;">
      <div style="text-align: center; margin-bottom: 20px;">
        <?php if ($unsubscribeUrl): ?>
          <a href="<?=$unsubscribeUrl ?>" style="display: inline-block; vertical-align: middle; text-decoration: none; font-family: Arial, Helvetica, sans-serif; font-size: 14px; line-height: 18px; color: #2b75d1;"><?= Yii::_t('partners.main.unsubscribe') ?></a>
        <?php endif ?>
      </div>
